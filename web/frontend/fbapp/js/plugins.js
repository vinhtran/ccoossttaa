function puzzle(wrapper,result,timeBox) {
	var wrapper = wrapper,
		obj = ".Piece",
		path = "pieces",
		result = result,
		...
		*/
		...
		for(var i = 0; i < 3; i++) {
			...
			for(var z = 0; z < 3; z++) {
				...
				imgs+= '<img id="p' + k +'" class="Piece ' + lastImgClass + '" src="' + path + '/' + resultArray[k] + '.png" width="' + wp + '" height="' + wp + '" style="left: ' + x + 'px; top: ' + y + 'px; " />';
				k++;
			}
		}
	...
	var startTimer = function() {
		...
		interval = setInterval(function() {
			...
			if(countUp > limitTime) {
				stopTimer();
				alert("Game Over! Please try again tomorrow.");
				// disable drag
				wrapper.children().draggable({
					disabled: true
				}).droppable({
